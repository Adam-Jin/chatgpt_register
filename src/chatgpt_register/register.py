...
from . import landbridge_runtime as _landbridge
_landbridge.configure(LANDBRIDGE_CFG, str(_paths.config_path()))
...
def _persist_codex_token_data(token_data: dict, filename_hint: str = "",
                              upload_to_cpa=None):
    token_dir = str(_paths.ensure_dir(_paths.token_dir_path(TOKEN_JSON_DIR)))
...
def _append_pending_oauth(email: str, password: str, email_pwd: str, mail_provider: str = ""):
    ...
    """
    try:
        path = str(_paths.ensure_parent(_paths.pending_oauth_path(PENDING_OAUTH_FILE)))
        with _file_lock:
...
def _read_pending_oauth(input_file: str):
    ...
    跳过空行/注释/格式不全的行。
    """
    path = str(_paths.pending_oauth_path(input_file))
...
def retry_oauth_only(input_file: str = PENDING_OAUTH_FILE,
    ...
    """
    output_file = str(_paths.output_file_path(output_file))
    ...
    if success_emails:
        out_path = str(_paths.ensure_parent(_paths.output_file_path(output_file)))
        with _file_lock:
...
def run_batch(total_accounts: int = 1, output_file="registered_accounts.txt",
    ...
    """
    mail_provider = _normalize_mail_provider(mail_provider)
    output_file = str(_paths.ensure_parent(_paths.output_file_path(output_file)))
        ...
        if not skip_solver_check:
            ...
            if ok:
                ...
                if SENTINEL_INPROCESS:
                    ...
                    _console_log("           请先启动: sentinel-solver --thread 2", level="warn")
